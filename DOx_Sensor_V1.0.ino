...
void setup() {
  ...
  GV_LCD_MAIN_TEXT[0]=PadString15chars(String(IP[0]) + '.' + String(IP[1]) + '.' + String(IP[2]) + '.' + String(IP[3]));
...
void loop() {
  ...
  if (GV_QUERY_DO_NAME_ON_NEXT_COMMAND){              // query DO name if this is set to true, then display it on the LCD screen. 
    ...
    GV_DOX_DATA.remove(13);
    GV_LCD_MAIN_TEXT[1]=PadString15chars(GV_DOX_DATA);
  ...
  if (((millis() - DOxReadingMillis) > 2000) && !GV_WEB_REQUEST_IN_PROGRESS && !GV_BOOTING_UP){
    GV_READ_REQUEST_IN_PROGRESS=true;
    SendCommandAndSetDOxVariables(ReadDOx);
    GV_DOX_DATA=PadString15chars(GV_DOX_DATA);
...
String PadString15chars(String str){
  char newStr[16];
  int c;
  for (c=0; c <= 14; c++){
    newStr[c]=str[c];
...
void SendCommandAndSetDOxVariables(String command) {
  ...
  command[0] = tolower(command[0]);           //set first char of command to lower case - used to test for what command was entered later in this function.
